    ...
    <a-layout-content style="padding: 0 50px">
      ...
      <div :style="{ background: '#fff', padding: '24px', minHeight: '430px' }">
        ...
        <a-form-model :model="form" form="form">
          <a-textarea v-model="content" :rows="9" />
          <br/><br/>
          <a-button type="primary" style="float: right" @click="submitForm('form')">提交</a-button>&nbsp;
        </a-form-model>
      </div>
    </a-layout-content>
...
export default {
  ...
  data(){
    return{
      content:this.$route.params.question_content,
      question_id:this.$route.params.question_id,
      title:this.$route.params.question_title,
      //title:'你好鸭',
  ...
  methods:{
    ...
    submitForm(formName){
      var _this = this;
      _this.$axios.post('/api/quest/alterQuestContent',
          _this.$qs.stringify({
        ...
        _this.$message.success('成功修改问题内容');
        _this.$router.push({
          path:'/personQuestion',
          name:'personQuestion',//路由命名
          params:{
            id:_this.id,
            img:_this.img,
            token:_this.token,
        ...
        _this.$message.error('请求失败，请重新提交');
      })
