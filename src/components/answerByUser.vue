<template>
  ...
  <a-layout-content style="padding: 0 50px">
    ...
    <div :style="{ background: '#fff', padding: '24px', minHeight: '430px' }">
      Content
      <div v-for="item in list" :key="item.message">
      <h2 style="font-weight: bold">{{item.question_title}}</h2>
        <br/><br/>
...
export default {
  ...
  data(){
    return {
      userId: this.$route.params.id,//传用户账号
      list:[],
  ...
  mounted() {
      var _this = this;
      _this.$axios.post('/api/showAnswerById',
          _this.$qs.stringify( {
